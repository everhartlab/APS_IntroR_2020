...
# Now we are ready to add a new column to our data frame `summer` so that it
# looks like this:
# 
# ```
#   day   month   year  wkday
...
# "Sat", "Sun" (since June starts on a Monday this year). We also know that we
# need that list to repeat until the total length of the list is equal to the
# number of days in June and June, which can be determined by using the `nrow()`
# function. 
# 

eachday <- c("Mon", "Tues", "Wed", "Thurs", "Fri", "Sat", "Sun")
wkday   <- rep(eachday, times = nrow(summer))
wkday
...
# examples in the help page for rep (type `?rep`), we can see that we need to
# use the argument `length.out`

wkday <- rep(eachday, length.out = nrow(summer))

# Inspect your new object `wkday` and make sure it's the correct length.

length(wkday) == nrow(summer)
wkday

# Now we are ready to add this vector to our data frame. We can do this by
# using `cbind()` or specifying the name of the column we want to add.
